...
router.get('/getGoldWeb/:u', function (req, res, next) {
  ...
  if (u.indexOf('https') != -1) {
    comm.geturlbyhttps(u, 'utf-8', function (val) {
      ...
      var $ = cheerio.load(val.toString());
      data.title = $('.art_tit_h1').text() || '';
      var cont = '';
      $('.art_p').each(function (element) {
        if ($(this).text())
          cont += '<p class="art_p">' + $(this).text() + '</p>'
      })
      data.content = cont;
      res.json(data);
    })
...
function getSXData() {
  var newsurl = 'http://interface.sina.cn/wap_api/layout_col.d.json?showcid=34977&col=34977&level=1%2C2&show_num=200&page=2&act=more&jsoncallback=feed_lotto_2551_1_3596649389618'
  var dcount = 0;
